    ...
    public function index (Request $req) {
        ...
        foreach ($users as $usr) {
            ...
            while ($tanggal->lte($end)) {
                ...
                if($jenis_presensi == 'absen_biasa_lembur') {
                    $harian = $harian->whereIn('type', ['absen_biasa', 'absen_lembur','absen_izin']);
                    // $harian = $harian->whereNotNull('id_jenis_lembur');
                }
    ...
    public function export(Request $req) {
        ...
        foreach ($users as $usr) {
            ...
            while ($tanggal->lte($end)) {
                ...
                $cek_telat = InOut::where('employee_id', $usr->id);
                $cek_telat = $cek_telat->whereIn('type', ['absen_biasa']);
                $cek_telat = $cek_telat->where('date', $tanggal->format('Y-m-d'));
                $cek_telat = $cek_telat->where('is_active', 1);
                $cek_telat = $cek_telat->first()->late ?? null;

                $tdcolor_telat = 'color:black;';
                if($cek_telat != null) {
                    $tdcolor_telat = 'color:#FD826C;';
                }

                $libur_cek = \App\Helpers\General::tanggalMerah($tanggal->format('Y-m-d'));
                if ($libur_cek == true) {
                    $tr = 'background:#DF2504;'.$tdcolor_telat;
                    $ket = '';
                } else {
                    $tr = $tdcolor_telat;
                    $ket = '';
        ...
        if($tipe_file == 'excel') 
        {
            ...
            ->download('laporan rekap e-kehadiran priode '. date('d-m-Y', strtotime($from_date)) .' sampai '. date('d-m-Y', strtotime($to_date)) .' dibuat tanggal '.date('d-m-Y').'.xlsx', \Maatwebsite\Excel\Excel::XLSX);
        } 
    ...
    public function detailReportPerKaryawan (Request $request) {
        ...
        for ($date = $start; $date->lte($end); $date->addDay()) {
            ...
            foreach($jenis_lembur as $lembur) {
                ...
                $list_lembur = $list_lembur->where('type', 'absen_lembur')->get();
                
                foreach($list_lembur as $key => $lb) {
                    $list_total_jenis_lembur[$lembur->slug][$key] = [
                        // 'jenis' => $lembur->type,
                        'date_in' => $lb->date ??  date('Y-m-d', strtotime($lb->created_at)),
                        'in' => $lb->clock_in ?? null,
                        'date_out' => $lb->date_out ?? date('Y-m-d', strtotime($lb->updated_at)),
                        'out' => $lb->clock_out ?? null,
                    ];
                }
